...
export default function Home() {
  return <main className="home-page">
    <section className="intro-stage" aria-labelledby="intro-title">
      <div className="intro-top"><span className="intro-brand"><span className="intro-monogram">F.</span><span>Oluwalana Faith</span></span><span className="intro-available"><i /> Available for new opportunities</span></div>
      <div className="intro-center"><Link className="latest-project" href={projects[0].href} target="_blank" rel="noreferrer"><span className="latest-thumb"><Image src={projects[0].image} alt="" fill sizes="34px" /></span><span>Explore my latest project</span><span aria-hidden="true">↗</span></Link><p className="intro-kicker">SOFTWARE DEVELOPER · PRODUCT BUILDER</p><h1 id="intro-title">Hi, I&apos;m Oluwalana Faith.<br /><span>Building products that <em>matter.</em></span></h1><p className="intro-description">I design and build thoughtful digital experiences across AI, Web3, data and the modern web.</p></div>
      <div className="project-showcase" aria-label="Featured projects"><div className="showcase-track">{[...projects, ...projects].map((project, index) => <a className={`showcase-tile showcase-${project.slug}`} key={`${project.slug}-${index}`} href={project.href} target="_blank" rel="noreferrer" aria-label={`Visit ${project.name} live site`} tabIndex={index >= projects.length ? -1 : 0}><Image src={project.image} alt="" fill sizes="(max-width: 650px) 160px, 205px" /><span className="showcase-caption">{project.name} <span aria-hidden="true">↗</span></span></a>)}</div></div><div className="intro-bottom"><span>SELECTED WORK / 01—04</span><Link href="/work">View all projects ↗</Link></div>
    </section>
